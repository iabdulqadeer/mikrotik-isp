@extends('layouts.app', ['title' => 'New Plan Price'])

@section('content')
<div class="max-w-3xl mx-auto p-4 space-y-4">
  <h1 class="text-xl font-semibold">Create Plan Price</h1>

  <form method="POST" action="{{ route('plan-prices.store') }}" class="space-y-4">
    @csrf
    ...
    <div class="grid sm:grid-cols-2 gap-4">
      <div>
        ...
        <input name="stripe_price_id" class="w-full rounded-lg border px-3 py-2" required />
        @error('stripe_price_id') <div class="text-red-600 text-sm mt-1">{{ $message }}</div> @enderror
      </div>
      <div>
        <label class="block text-sm font-medium mb-1">Currency</label>
        <input name="currency" value="usd" class="w-full rounded-lg border px-3 py-2" />
    ...
    <div class="grid sm:grid-cols-3 gap-4">
      <div>
        ...
        <input type="number" name="amount" class="w-full rounded-lg border px-3 py-2" min="0" required />
      ...
      <div>
        ...
        <select name="interval" class="w-full rounded-lg border px-3 py-2">
          <option>month</option><option>year</option><option>week</option><option>day</option>
        </select>
      ...
      <div>
        ...
        <input type="number" name="interval_count" value="1" class="w-full rounded-lg border px-3 py-2" min="1" />
    ...
    <div>
      ...
      <input name="role_name" class="w-full rounded-lg border px-3 py-2" />
    ...
    <div>
      ...
      <textarea name="features" class="w-full rounded-lg border px-3 py-2" rows="3"
        placeholder='["X Mbps","Priority support"]'></textarea>
    ...
    <div class="pt-2">
      <button class="px-4 py-2 rounded-lg bg-indigo-600 text-white hover:bg-indigo-700">Save</button>
